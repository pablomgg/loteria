...
public class LotofacilService(
    ILogger<LotofacilService> logger,
    IServiceScopeFactory scopeFactory,
    ILotofacilApi lotofacilApi) : IJogoImportService
{
    public TipoJogoLoteria Tipo => TipoJogoLoteria.Lotofacil;
    ...
    public async Task ImportAsync(CancellationToken cancellationToken)
    {
        ...
        var lastNumberDb = await GetLastLotofacilAsync(db, cancellationToken);
        var lastApi = await GetLastLotofacilApiAsync(cancellationToken);
        
        logger.LogInformation("Lotofácil -> banco: {db} | api: {api}", lastNumberDb, lastApi?.NumeroConcurso);

        if (lastApi?.NumeroConcurso <= lastNumberDb)
        {
            logger.LogInformation("Lotofácil já está atualizada.");
            return;
        }
        ...
        logger.LogInformation("Lotofácil -> importando de {start} até {end}", start, end);

        await ImportRangeAsync(db, start, end, cancellationToken);

        logger.LogInformation("Lotofácil -> import finalizado.");
    }
    
    private async Task<int> GetLastLotofacilAsync(AppDbContext db, CancellationToken cancellationToken)
    {
        var numeroConcurso = await db.Set<Concurso>()
            .Where(c => c.TipoJogo == TipoJogoLoteria.Lotofacil)
            .MaxAsync(c => (int?)c.NumeroConcurso, cancellationToken) ?? 0;

        return numeroConcurso;
    }
    
    private async Task<Concurso?> GetLastLotofacilApiAsync(CancellationToken cancellationToken)
    {
        try
        {
            return await lotofacilApi.GetLatestAsync(cancellationToken);
        }
    ...
    private async Task ImportRangeAsync(AppDbContext db, int start, int end, CancellationToken cancellationToken)
    {
       ...
       try
       {
           ...
           for (int concursoNumero = start; concursoNumero <= end; concursoNumero++)
           {
               ...
               if (dto is null)
               {
                   logger.LogWarning("Lotofacil concurso {n}: não encontrado (404?)", concursoNumero);
                   continue;
               }
               ...
               if (batch.Count >= SaveBatchSize)
               {
                   ...
                   logger.LogInformation(
                       "Lotofacil -> lote salvo com {batchCount} concursos. Total processados: {p} (ins {i} | upd {u})",
           ...
           if (batch.Count > 0)
           {
               ...
               logger.LogInformation(
                   "Lotofacil -> lote final salvo com {batchCount} concursos. Total processados: {p} (ins {i} | upd {u})",
                   batch.Count, processed, inserted, updated);

               batch.Clear();
           }

           logger.LogInformation("Resumo Lotofacil: processados {p} | inseridos {i} | atualizados {u}",
    ...
    private async Task<Lotofacil?> TryGetByNumberAsync(int numero, CancellationToken cancellationToken)
    {
        try
        {
            return await lotofacilApi.GetByNumberAsync(numero, cancellationToken);
        }
        catch (ApiException ex) when ((int)ex.StatusCode == 404)
        {
    ...
    private async Task UpdateAsync(AppDbContext db, long id, Concurso incoming, CancellationToken cancellationToken)
    {
        ...
        if (incoming.Lotofacil is not null)
        {
            if (current.Lotofacil is null)
                current.Lotofacil = new LotofacilDetalhe { ConcursoId = id };

            current.Lotofacil.Acumulado = incoming.Lotofacil.Acumulado;
            current.Lotofacil.IndicadorConcursoEspecial = incoming.Lotofacil.IndicadorConcursoEspecial;
            current.Lotofacil.Observacao = incoming.Lotofacil.Observacao;
            current.Lotofacil.ValorArrecadado = incoming.Lotofacil.ValorArrecadado;
        }
